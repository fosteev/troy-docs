...
Моб действует по **запрограммированному боевому алгоритму** — упорядоченному по приоритету
списку своих скиллов с условиями применения. Автоатака — независимый базовый ритм (по
`attack_speed`), который идёт сам по себе в промежутках; алгоритм решает, какой **скилл**
наложить, когда несколько готовы.

### Алгоритм выбора скилла

Каждый тик, если моб не в касте и не в стане, он проходит свои скиллы по приоритету и берёт
**первый**, который одновременно: (1) не на кулдауне и (2) удовлетворяет своему условию.
Если ни один не подошёл — скилл в этот тик не применяется (работает автоатака по своему таймеру).

```
skills = mob.skills, отсортированы по priority (меньше = раньше)

each tick (моб не в касте и не в стане):
  for skill in skills:
    if skill.cooldownReady AND conditionMet(skill, state):
      use(skill); break        // инстант — сразу; cast_time > 0 — уходит в каст
  // ничего не подошло → ждём; автоатака сработает по своему таймеру
```

### Условия применения (`condition`)

| condition | conditionValue | Срабатывает, когда |
|---|---|---|
| `always` | — | всегда, как только скилл готов |
| `opener` | — | только в самом начале боя (стартовая «открывашка»), один раз за бой |
| `self_hp_below` | pct (0–100) | `mob.hp / mob.maxHp * 100 < pct` — фаза «ярости»/паники |
| `target_hp_below` | pct (0–100) | `player.hp / player.maxHp * 100 < pct` — попытка добить |

Набор расширяемый: новое поведение = новое значение `condition` + ветка в эвалюаторе.
Для MVP хватает этих четырёх.

### Взаимодействие с автоатакой, кастом, станом

- **Автоатака независима** — идёт по `1 / attack_speed` и заполняет промежутки между скиллами
  (как у игрока). Скилл с `cast_time > 0` блокирует автоатаку на время каста; инстант — нет.
- **Каст/стан:** в стане моб не выбирает скиллы и не автоатакует; во время собственного каста
  новый скилл не начинает. Стан во время каста срывает каст (скилл уходит на 50% кулдауна).
- **Без ресурсов:** у мобов нет Rage/Mana — скиллы ограничены только кулдауном и условием.

### Данные алгоритма (на скилле моба)

Алгоритм хранится прямо на скиллах моба (`MonsterSkill`): порядок оценки + условие.

| Поле | Описание |
|---|---|
| `priority` | порядок оценки, меньше — раньше (в БД переиспользуем существующий `sortOrder`) |
| `condition` | `always` / `opener` / `self_hp_below` / `target_hp_below` |
| `conditionValue` | порог в % для `*_hp_below` (иначе null) |
| `cooldownSec`, `castTimeSec`, `baseDamage`, `scalingStat`, `scalingRatio`, `effectType`, `effectValue`, `effectDurationSec` | как раньше |

### Примеры алгоритмов

**Brute (берсерк):**
1. `opener` → Stun Slam — оглушить в начале боя
2. `self_hp_below:30` → Enrage — buff на урон, когда сам при смерти
3. `always` → Heavy Swing — сильный удар по кулдауну

**Shaman (кастер):**
1. `target_hp_below:35` → Finisher Bolt — добить игрока
2. `self_hp_below:50` → Heal — подлечиться
3. `always` → Fire Bolt — базовый нюк по кулдауну

(Конкретные мобы, числа и наборы — контент Шага 5, `monsters.md`.)
...
- Убитый моб исчезает **только для этого персонажа**, а не для всех. Факт убийства
  записывается на персонаже по уникальному ID спавна (`active_spawns.id`) — для
  остальных игроков моб остаётся на карте, любой может начать по нему свой бой.
- Прогресс убийств сбрасывается **еженедельно: каждую среду в 00:00 UTC** — после
  сброса все мобы снова видны всем. Реализация: убийство учитывается, если
  `killedAt >= начала текущей недели` (среда 00:00 UTC), отдельная джоба-truncate
  не требуется.
