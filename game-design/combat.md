...
Маг входит в бой с **полной маной**. Это жёсткое правило инициализации: при каждом старте боя выполняется `currentMana = maxMana`. Мана тратится на скиллы и восстанавливается со временем:

```
Max_Mana = base_mana + INT * 8

Регенерация в бою:  max_mana * 0.01 * (1 + SPI * 0.02) per second
Регенерация вне боя: max_mana * 0.03 * (1 + SPI * 0.02) per second
```
...
- После боя мгновенно восстанавливается:
  - `HP += 10% max_hp`
- Rage сбрасывается в 0
...
- После окончания `incapacitated`: `HP = 30% max_hp`, `Mana = 30% max_mana` (для Mage), `Rage = 0` (для Warrior)
- При следующем входе в бой для Mage всё равно применяется правило старта боя: `currentMana = maxMana`
...
- Успешный побег завершает бой без XP/лута, моб возвращается к полному HP, Rage у Warrior сбрасывается в 0
...
mana_regen = max_mana * 0.03 * (1 + SPI * 0.02) per second (Mage)
rage = 0 (Warrior, сбрасывается)
```

Регенерация вне боя обновляет состояние между боями, но в момент старта боя у Mage всегда выполняется `currentMana = maxMana`.
