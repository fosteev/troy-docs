...
Мана существует постоянно. Тратится на скиллы, восстанавливается со временем.

```
Max_Mana = base_mana + INT * 8

Регенерация маны:
  Вне боя:  mana_regen = max_mana * 0.03 * (1 + SPI * 0.02) per second
  В бою:    mana_regen = max_mana * 0.01 * (1 + SPI * 0.02) per second

После списания/регенерации:
  mana = clamp(mana, 0, max_mana)
```

Spirit влияет на скорость регенерации маны. При SPI=20 множитель = 1.4 (+40% реген).
...
| **Phys Attack** | `base_atk + STR × 2` |
| **Magic Attack** | `base_matk + INT × 2.5` |
