...
- [ ] **Шаг 3Б** — Battle polish: отзывчивость и «живость» боя — см. [battle-polish.md](./battle-polish.md) — **P0, P1, P4, P3 done** (контракт событий; отзывчивость/читаемость/устойчивость; перекладка экрана в «Арену»; старт/результат/level-up). Осталось: **P5** — лента намерений моба (нужен контракт `BattleMonsterDto.skills`) и **P2** — звук/VFX/фоны арен/спрайты смерти мобов, заблокирован ассетами (нет ни одного SFX, нет полей `Monster.spriteHit`/`spriteDeath` и `SpawnZone.arenaBackground`)
