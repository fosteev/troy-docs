...
- [ ] Миграция `0012_monster_sprites` написана руками, `prisma migrate dev` не запускался.
- [ ] `npm run prisma:generate` выполнен, backend собирается: `npm run build`.
- [ ] Все backend-тесты зелёные: `npx nx run-many -t test` (включая новые).
- [ ] Admin CRUD мобов принимает/возвращает `spriteIdle`/`spriteAttack`; replace скиллов — `spriteAttack` на скилл; невалидный спрайт отклоняется с понятной ошибкой.
- [ ] `GET /monsters/:id/visuals` (JWT) отдаёт `{ id, name, iconUrl, spriteIdle, spriteAttack, skills: [{ code, spriteAttack }] }`, на неизвестный id — 404; эндпоинт виден в Swagger.
- [ ] В админке: у моба два спрайт-слота с загрузкой/превью (как у классов), у каждого скилла — свой спрайт через модалку, в списке мобов — теги наличия спрайтов.
- [ ] Страницы классов работают как раньше (компоненты переехали без регрессий), `npm run build` админки зелёный.
- [ ] Никаких новых зависимостей, никакого рефакторинга вне описанных шагов.
